...
/**
 * Clears command caches.
 * When a live update occurs on the page, we want to respond by destroying our command caches
 * to reflect the state of the page.
 */
function clearCommandCaches(commandPalette: CommandPalette) {
  ...
  observe('.js-socket-channel.js-updatable-content', {
    subscribe: el =>
      fromEvent(el, 'socket:message', () => {
        ...
        clearCommandCaches(commandPalette)
      })
  })
}
